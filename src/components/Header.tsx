...
export const Header: React.FC<HeaderProps> = ({ setMobileOpen }) => {
  ...
  return (
    <header className="pos-header sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
      {/* Left section */}
          ...
          {userMenuOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white rounded-3xl shadow-2xl border border-slate-200/90 py-2.5 z-50 animate-in fade-in zoom-in-95">
              <div className="px-3.5 py-2 border-b border-slate-100">
                <div className="text-xs font-bold text-slate-800">{currentUser?.name || 'Administrator'}</div>
                <div className="text-[11px] text-slate-500">@{currentUser?.username || 'admin'}</div>
                                <div className="text-[11px] text-slate-500">@{currentUser?.username || 'admin'}</div>
                <div className="mt-1">
